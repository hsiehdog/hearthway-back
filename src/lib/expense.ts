...
type ParticipantCostMap = Record<string, string>;
...
const calculateParticipantCosts = (
  ...
  if (expense.splitType === SplitType.EVEN) {
    ...
    participants.forEach((participant) => {
      shareCountsByMember[participant.memberId] = toDecimal(
  ...
  return participants.reduce<ParticipantCostMap>((acc, participant) => {
    const rawShare =
      shareCountsByMember[participant.memberId] ?? new Prisma.Decimal(0);
    let cost = evenShare;

    if (expense.splitType === SplitType.PERCENT && rawShare.gt(0)) {
      cost = amount.mul(rawShare).div(100);
    } else if (
      expense.splitType !== SplitType.PERCENT &&
      totalShares.gt(0) &&
      rawShare.gt(0)
    ) {
